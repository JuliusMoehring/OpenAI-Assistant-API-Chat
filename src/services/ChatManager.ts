...
import { submitUserMessage, fetchAssistantResponse, updateChatState } from "../modules/chatModules";
...
class ChatManager {
    ...
    async sendMessage(input: string, files: File[], fileDetails: any[]): Promise<void> {
            ...
            if (this.state.threadId && this.state.assistantId) {
                ...
                if (files.length > 0) {
                    ...
                    ChatFileIds = await Promise.all(
                        files.map((file) => prepareUploadFile(file, this.state.setStatusMessage)),
                    );
